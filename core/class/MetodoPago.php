<?php
	class MetodoPago{
		...
		public function listar($key){
			...
			$sql = 'SELECT * from metodo_pago where status = "active"';
		...
		public function ver($key, $id){
			$connection = new Connection;
			$connection->abrir($key);
			$sql = 'SELECT * from metodo_pago where clave = "'.$id.'" and status = "active"';
			$res = $connection->consultaDatos($key,$sql);
			return ($res);
		}
		...
		public function agregar($key,$atr){
			$connection = new Connection;
			$connection->abrir($key);
			$sql = 'INSERT INTO metodo_pago(clave) values (upper("'.$atr['id'].'"))';
		...
		public function editar($key, $id, $atr){
			$connection = new Connection;
			$connection->abrir($key);
			$sql = 'UPDATE metodo_pago SET clave = upper("'.$atr['id'].'") where clave = "'.$id.'"';
			$res = $connection->disparadorSimple($key,$sql);
			return ($res);
		}
		...
		public function papelera($key){
			$connection = new Connection;
			$connection->abrir($key);
			$sql = 'SELECT * from metodo_pago where status = "inactive"';
			$res = $connection->consultaDatos($key,$sql);
			return ($res);
		}
